<?php
/**
 * Hide parent Contact from the Page Components "Add Component" dropdown.
 *
 * Admin only — removing a layout definition on the front end would break
 * rendering of any page that still has a Contact row saved.
 *
...
add_filter('acf/load_field/name=page_components', function ($field) {
    ...
    $field['layouts'] = array_values(array_filter(
        $field['layouts'],
        static fn ($layout) => ($layout['name'] ?? '') !== 'Contact'
    ));
    return $field;
});
